...
public class ToyLand {
    ...
    public void playTime() {
        IToy curToy;
        while (activeToys.size() > 0){
            curToy = activeToys.get(0);
            activeToys.remove(0);
            curToy.play(getRandomTime());
            if (curToy.isRetired())
                retiredToys.add(curToy);
            else
                activeToys.add(curToy);
        }
    }
    ...
    public void displayStatistics() {
        ...
        for (IToy retired: retiredToys){
            System.out.println("\t" + retired);
            totalHappiness += retired.getHappiness();
            totalWear += retired.getWear();
        }
        System.out.println(
                "Total toys: " + retiredToys.size() + "\n" +
                "Total happiness: " + totalHappiness + "\n" +
                "Average wear: " + totalWear / retiredToys.size()
        );
    }
